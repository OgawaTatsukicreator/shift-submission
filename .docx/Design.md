...
| 下書き | 従業員IDと対象月で分離し、認証トークンを除外した | 更新日時、30日保持、期限切れ理由の表示を追加する |
...
| パスワード変更 | パスワード版、旧版セッション拒否、全有効セッション失効、サマリ同期失敗時の復元を追加した | 認証操作ID、工程記録、決定的セッション再発行を追加する |
...
| エラー表示 | 内部例外を定型文へ置換し、20秒無応答後に同じ操作IDを最大2分照会する。未保存、部分保存、保存済みを基本区別する | 全RPC追跡ID、技術エラーログ、追記履歴、完全なリース判定を追加する |
...
| 提出ログ用 | `SUBMISSION_LOG_SPREADSHEET_ID` | 現行の `シフト希望`, `PT申請`, `ログインアカウント`, `ログインセッション`, 基本 `操作処理状態`。目標版では `初回登録コード`, `技術エラーログ` も配置する |
...
| 最優先 | 初回登録に一回限りの登録コード、操作ID、回復トークン、`PENDING` 状態がない | 従業員IDを知る第三者による先取り登録と中断復旧に対応できない |
...
| 高 | 下書きに更新日時と30日の保存期限がなく、期限切れ理由を表示しない | 共有端末へ古い入力が残り続ける可能性がある |
