...
| エラー表示 | 内部例外を定型文へ置換し、20秒無応答後に同じ操作IDを最大2分照会する。未保存、部分保存、保存済みを基本区別し、希望提出・PT申請の保存障害へ追跡IDを付けて分類済みの技術エラーログを残し、90日を超えた行を日次処理で削除する。列見出し不一致と破損した希望JSONはデータを変えず停止する | 全RPC追跡ID、スキーマ版、追記履歴、完全なリース判定を追加する |
| 店休日 | `業務設定` の `STORE_CLOSED_RULE` から全体・店舗別の休業曜日と休業日を判定し、店舗別設定を全体設定より優先する | 締切、タイムゾーンなど残るコード設定も版付き業務設定へ移す |

現行GAS版UIの判定対象は `GasApp.html` とする。`index.html`、`styles.css`、`app.js` は入力・保存・通信を行わない旧版利用停止案内であり、本設計の実装確認には使用しない。
...
| 高 | 操作状態の保持期限、追記専用履歴、全RPCの追跡IDが未実装 | 提出以外の障害調査と長時間中断後の整合確認が不足する |
