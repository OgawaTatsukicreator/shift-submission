...
| エラー表示 | 内部例外を定型文へ置換し、20秒無応答後に同じ操作IDを最大2分照会する。未保存、部分保存、保存済みを基本区別し、希望提出・PT申請の保存障害へ追跡IDを付けて分類済みの技術エラーログを残す。列見出し不一致と破損した希望JSONはデータを変えず停止する | 全RPC追跡ID、ログ保持期限、スキーマ版、追記履歴、完全なリース判定を追加する |
...
| 高 | 操作状態の保持期限、追記専用履歴、全RPCの追跡ID、技術エラーログの90日自動削除が未実装 | 提出以外の障害調査と長時間中断後の整合確認が不足する |
