...
- 現行コードは `管理対象ファイル` シートを優先し、該当行がない場合にコード内のID定数へフォールバックする。一部の確定元取得はエリアIDを渡すが、通常の全体・エリア表示・提出ログ取得は管理単位の先頭有効行を使い、店舗IDは接続先解決に使わない。目標版、ロールバック互換版、検証版は、固定DBを環境マニフェストから読み、他の接続先を有効な設定版から管理単位・エリアID・店舗IDごとに解決する。コード定数へのフォールバックを禁止し、環境IDをアプリ本体から分離する。
- セル色は見やすさと確定判定の補助に使う。正データは `確定シフト` シートに保存する。
- 従業員向けWebアプリは提出済み希望だけを表示し、現段階では `確定シフト` を取得・表示しない。

## 現行実装の検証結果

2026年9月19日に `GasApp.html` と `Code.gs` を静的確認し、Windows版Chromiumのブラウザズーム100%、テスト用氏名「テスト従業員」で320×700 CSS pxと390×844 CSS pxのローカル簡易計測を行った。この簡易計測ではChromium版、既定フォント、OS表示倍率、`devicePixelRatio`、`window.innerWidth/innerHeight`、画面画像、試験ログを保存していないため、寸法は参考値であり受入判定には使わない。基本的な提出・認証・本人履歴確認のコードは存在するが、GASと実スプレッドシートを使った結合動作は未検証である。現行実装は本設計へ完全には適合していない。

| 分類 | 現状 | 設計上の対応 |
| --- | --- | --- |
| スマートフォンUI | ローカル簡易計測の参考値では、320 CSS px幅の日別カード内の開始・終了選択が各27.5×46 CSS px、固定ヘッダーが高さ208pxで700px高の画面の29.7%を占めた。受入条件どおりの環境記録と証跡は未取得 | タップ対象の最小寸法と間隔を優先して列数を切り替え、固定領域を主操作中心に再構成する |
| 下書き | 従業員単位のキーへ対象月を含むペイロード全体を保存し、認証トークンも含まれる | 従業員と対象月のキーに分離し、認証情報を除外する |
| 書き込み | 希望提出、PT申請、確定反映に共通の操作ID、排他制御、処理段階の記録がない | 冪等処理、ScriptLock内での業務キー予約、処理状態、再開手順を追加する |
| 認可 | 従業員向けAPIはセッションを検証するが、複数の管理関数は役割を検証しない | 公開RPCごとに必要ロールを定義し、サーバー側で検証する |
| パスワード変更 | 通常の単一セッション行は新トークンで上書きされるが、全行失効処理とパスワード版がないため、重複セッション行やパスワード保存後から新セッション発行前の中断時に旧セッション失効を保証できない。パスワードサマリ同期の失敗も記録するだけで成功応答を返す | パスワード版による即時失効と、工程記録・再開可能な全行失効・サマリ同期を追加する |
...
| 確定反映 | 第1週かつ最初の役職グループだけを読み取り、解除済み行が残る。同じ従業員・店舗・日付に希望と確定があると、先に格納された希望が選ばれる。赤セルも `休み` の確定行として保存する | 全件事前検証、版付きスナップショット、確定データ優先のキー結合へ変更し、赤セルは確定勤務行から除外する |
| エラー表示 | 入力保持と通信失敗表示はあるが、保存済み、未保存、結果不明、部分保存を区別しない | 共通レスポンスと保存状態を導入し、次の行動を明示する |
...
| 提出ログ用 | `SUBMISSION_LOG_SPREADSHEET_ID` | `シフト希望`, `PT申請`, `ログインアカウント`, `ログインセッション`, `初回登録コード`, `技術エラーログ` |
...
| 優先度 | 未達事項 | 影響 |
| --- | --- | --- |
| 最優先 | 下書きにセッショントークンを含め、PT申請ペイロードをコンソールへ出力している | 共有端末やログからセッションを再利用される可能性がある |
| 最優先 | 管理関数にサーバー側の役割認可がない | 従業員向けWebアプリから管理処理を呼び出される可能性がある |
| 最優先 | パスワード変更時、通常の単一セッション行は新トークンで上書きするが、全行失効とパスワード版がなく、サマリ同期失敗を握り潰して成功応答を返す | 重複セッション行またはパスワード保存後の中断時に旧トークンが残り得て、管理用サマリの不一致も把握できない |
| 最優先 | 希望提出は正データ保存後の表示再生成失敗を提出失敗として返し、操作IDによる再開がない | 保存済みなのに再送され、結果の判断ができない |
| 最優先 | PT申請と確定反映が非原子的で、操作IDによる重複防止がない | 部分保存、重複申請、表示との不一致が発生し得る |
| 最優先 | 確定読取が全週・全役職を処理せず、解除済み行を除去しない | 確定シフトが欠落または残存する |
| 最優先 | `confirmMatrixSheet` が呼出し側指定の `sourceSpreadsheetId` を、有効設定版・担当エリア・許可済み入力元へ照合せず開く | 実行者がアクセスできる想定外のファイルを確定元として読み、誤った確定データを書き込める |
| 最優先 | 赤セルを `休み` の確定行として保存する | 赤セルを新しい有効版の確定行にしない目標設計と異なり、休みを確定データとして扱う可能性がある |
| 最優先 | 表示用データは希望行を確定行より先に連結し、同じ業務キーで最初の行を採用する | 希望が確定値より優先され、確定結果と画面表示が一致しない |
| 高 | 対象月、実在日、対象月内日付、重複日、入力区分のGAS側検証が不足している | 不正な希望データを保存し得る |
| 高 | `DEADLINE_DAY` は表示に使うだけで、GASの受付判定に使っていない | 締切後も受付できる |
| 高 | 希望提出、PT申請、確定反映、ログイン失敗回数更新に十分な排他制御がない | 同時実行で重複や更新消失が起こり得る |
| 高 | 初回登録が従業員IDの照合だけで完了し、一回限りの登録コードがない | 従業員IDを知る第三者が先に登録できる |
| 高 | 接続先ファイルを処理ごとに解決し、設定版と操作ごとの固定先がない | 切替中の多段処理が旧ファイルと新ファイルへ分断され得る |
| 高 | クライアントにタイムアウトと操作状態照会がなく、コールバックが返らないとボタンが処理中のままになる | 結果不明時に再送か待機かを判断できない |
| 高 | 一部の失敗で `error.message` をそのまま利用者へ表示する | 内部例外や構成情報が漏れる可能性がある |
| 高 | 必須ヘッダーの不一致を実行時に自動上書きする | 列ずれを隠し、運用中のデータを誤解釈する可能性がある |
| 高 | 壊れた希望JSONを空配列へ置き換えて処理する | 提出済みデータが未提出または空に見え、破損を検知できない |
| 高 | 確定対象セルの氏名、店舗、日付、値を解決できない場合に一部を黙って読み飛ばす | 不完全な確定結果を成功として公開し得る |
| 高 | 320〜375 CSS pxでも2列カードを維持する | 時刻選択が押しにくく、誤操作しやすい |
| 高 | 下書きキーが対象月を含まない | 月切替で別月の下書きを失う |
| 中 | `従業員店舗設定` の有効な `兼務`・`ヘルプ可` 関係をサーバー側の提出認可に使用していない | 想定外の店舗へ希望を提出できる |
| 中 | 全金曜日を全店休みとするコード内ルールが文書化・設定化されていない | 店舗ごとの営業ルールと食い違う可能性がある |
| 中 | `XFrameOptionsMode.ALLOWALL` を使用している | 外部埋め込みを禁止する目標要件と一致せず、意図しないサイトへ埋め込まれ得る |
